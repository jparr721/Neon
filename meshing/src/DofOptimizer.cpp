...
/// \param max Whether or not to apply to the max surface ("top") or min ("bottom")
/// \param mesh The uniform_mesh nodes we are interacting with
/// \param active_nodes All free nodes sorted
/// \param force_nodes All force-applied nodes sorted
/// \param fixed_nodes All fixed (unmoving) nodes sorted
void meshing::DofOptimizeUniaxial(const meshing::Axis axis, bool max, const std::shared_ptr<meshing::Mesh> &mesh,
                                  std::vector<unsigned int> &interior_nodes, std::vector<unsigned int> &force_nodes,
                                  std::vector<unsigned int> &fixed_nodes) {
    NEON_ASSERT_ERROR(axis != Axis::ALL, "Cannot apply uniaxial force to all axes");
    ...
    std::set_union(force_nodes.begin(), force_nodes.end(), fixed_nodes.begin(), fixed_nodes.end(),
                   std::back_inserter(ignored));

    // Active nodes are everything else
    FindInteriorNodes(ignored, mesh->positions.rows(), interior_nodes);
}

/// Creates a multi-axial distribution across distinct axes. This currently is bug prone due to conflicting axes
/// assigning nodes as fixed and un-fixed and will be fixed later.
/// \param axes
/// \param min_max
/// \param fixed_axes
/// \param min_max_fixed
...
/// \param force_nodes
/// \param fixed_nodes
void meshing::DofOptimizeMultiAxial(const std::vector<Axis> &axes, const std::vector<bool> &min_max,
    ...
    NEON_ASSERT_ERROR(axes.size() == min_max.size(), "Axes and axis specification vectors must be a bijection");
    NEON_ASSERT_ERROR(!(axes.size() > 6), "Cannot apply surface-wide forces in more than 2 places");
    ...
    for (int i = 0; i < axes.size(); ++i) {
        FindSurfaceNodes(axes.at(i), min_max.at(i), mesh, separated_force_nodes.at(i));
    }

    std::size_t separated_force_nodes_size = 0;
    for (const auto &node : separated_force_nodes) { separated_force_nodes_size += node.size(); }
    ...
    force_nodes.erase(std::unique(force_nodes.begin(), force_nodes.end()), force_nodes.end());

    // Find all fixed nodes
    std::vector<std::vector<unsigned int>> separated_fixed_nodes;
    separated_fixed_nodes.resize(axes.size());
    for (int i = 0; i < axes.size(); ++i) {
        FindSurfaceNodes(fixed_axes.at(i), min_max_fixed.at(i), mesh, separated_fixed_nodes.at(i));
    }

    std::size_t separated_fixed_nodes_size = 0;
    for (const auto &node : separated_fixed_nodes) { separated_fixed_nodes_size += node.size(); }
    ...
    fixed_nodes.erase(std::unique(fixed_nodes.begin(), fixed_nodes.end()), fixed_nodes.end());
...
void meshing::DofOptimizeClear(meshing::Axis axis, const bool max, const std::shared_ptr<meshing::Mesh> &mesh,
    ...
    if (axis == Axis::ALL) {
        ...
        all_active_nodes.insert(all_active_nodes.begin(), force_nodes.begin(), force_nodes.end());
    ...
    std::set_difference(force_nodes.begin(), force_nodes.end(), force_removed_nodes.begin(), force_removed_nodes.end(),
                        active_nodes.end());
}

void meshing::FindSurfaceNodes(meshing::Axis axis, const bool max, const std::shared_ptr<meshing::Mesh> &mesh,
                               std::vector<unsigned int> &nodes) {
    Real axis_coeff = max ? mesh->positions.col(axis).maxCoeff() : mesh->positions.col(axis).minCoeff();
...
void meshing::FindSurfaceNodes(std::vector<Axis> axes, const bool max, const std::shared_ptr<meshing::Mesh> &mesh,
                               std::vector<unsigned int> &nodes) {
    for (const auto &axis : axes) { FindSurfaceNodes(axis, max, mesh, nodes); }
}
...
/// \param total_nodes The number of total nodes we are putting into the interior
/// \param interior_nodes The interior nodes which are active but have no force applied
void meshing::FindInteriorNodes(const std::vector<unsigned int> &excluded, std::size_t total_nodes,
