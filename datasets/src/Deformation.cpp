...
void datasets::Deformation::Generate(const solvers::boundary_conditions::BoundaryConditions &boundary_conditions,
    ...
    for (Real v = min_v; v <= max_v; v += v_incr) {
        ...
        const auto E_ret = solvers::algorithms::FnBinarySearch<decltype(is_max_E), VectorXr, Real>(
                is_max_E, E_range, 0, E_range.rows() - 1, 0.5);
        NEON_ASSERT_WARN(E_ret.displacement > 0, "There exists no E and v combo that satisfies the "
...
void datasets::Deformation::GenerateSearchSpace(unsigned int shape, const int force_min, const int force_max,
                                                ...
                                                Real G_incr) {
    auto solver_controller = std::make_unique<solvers::controllers::SolverController>(shape);
    // My God...
                                ...
                                for (Real G_yz = min_G; G_yz < max_G; G_yz += G_incr) {
                                    for (Real G_zx = min_G; G_zx < max_G; G_zx += G_incr) {
                                        for (Real G_xy = min_G; G_xy < max_G; G_xy += G_incr) {
                                            ...
                                            solver_controller->Material() =
                                                    solvers::materials::OrthotropicMaterial(E, v, G);
                                            solver_controller->ReloadUniformSolver(
                                                    solvers::fem::LinearElastic::Type::kStatic);
                                            solver_controller->SolveUniform(
                                                    solvers::controllers::SolverController::kUseStaticSolver);
                                            Real avg_displacement = solver_controller->AverageDisplacement();
#pragma omp critical
                                            {
...
auto datasets::Deformation::DeformationBinarySearchReturnType::Ok() const -> bool {
    return solvers::numbers::IsApprox(displacement, target, epsilon);
}
