...
datasets::Deformation::Deformation(const std::string &path)
    : path_(path), csv_(path, {"E_x", "E_y", "E_z", "v_xy", "v_xz", "v_yz",
                               "G_yz", "G_zx", "G_xy", "Displacement"}) {}

void datasets::Deformation::Generate(
    const solvers::boundary_conditions::BoundaryConditions &boundary_conditions,
    const std::shared_ptr<meshing::Mesh> &mesh_, Real min_E, Real max_E,
    Real min_v, Real max_v, Real E_incr, Real v_incr) {
  ...
  for (Real v = min_v; v <= max_v; v += v_incr) {
    const auto is_max_E = [&](Real target,
                              Real E) -> DeformationBinarySearchReturnType {
      ...
      const auto static_solver = std::make_unique<solvers::fem::LinearElastic>(
          boundary_conditions, E, v, mesh_clone,
    ...
    const auto E_ret = utilities::algorithms::FnBinarySearch<decltype(is_max_E),
                                                             VectorXr, Real>(
        is_max_E, E_range, 0, E_range.rows() - 1, 0.5);
    NEON_ASSERT_WARN(E_ret.displacement > 0,
                     "There exists no E and v combo that satisfies the "
                     "displacement constraint");

    if (E_ret.displacement > 0) {
      Ev.emplace_back(E_ret.E, v, E_ret.displacement);
    }
  }

  for (const auto &c : Ev) {
    csv_ << std::vector<std::string>{std::to_string(std::get<0>(c)),
                                     std::to_string(std::get<1>(c)),
                                     std::to_string(std::get<2>(c))};
  }
}

void datasets::Deformation::GenerateSearchSpace(
    unsigned int shape, const int force_min, const int force_max,
  ...
  auto gen = std::make_unique<
      meshing::implicit_surfaces::ImplicitSurfaceGenerator<Real>>(shape, shape,
                                                                  shape);

  MatrixXr V;
  MatrixXi F;
  gen->GenerateImplicitFunctionBasedMaterial(
      meshing::implicit_surfaces::ImplicitSurfaceGenerator<Real>::kNoThickness,
      false, V, F);
  ...
  for (int force = force_min; force >= force_max; --force) {
    ...
    meshing::DofOptimizeUniaxial(meshing::Axis::Y, meshing::kMaxNodes, mesh,
                                 interior_nodes, force_nodes, fixed_nodes);
    solvers::boundary_conditions::LoadBoundaryConditions(
        Vector3r(0, force, 0), mesh, force_nodes, interior_nodes,
        boundary_conditions);
                ...
                for (Real G_yz = min_G; G_yz < max_G; G_yz += G_incr) {
                  for (Real G_zx = min_G; G_zx < max_G; G_zx += G_incr) {
                    for (Real G_xy = min_G; G_xy < max_G; G_xy += G_incr) {
                      ...
                      const auto material =
                          solvers::materials::OrthotropicMaterial(E, v, G);
                      const auto mesh_clone =
                          std::make_shared<meshing::Mesh>(*mesh);
                      const auto solver =
                          std::make_unique<solvers::fem::LinearElastic>(
                      ...
                      Real sum = 0;
                      for (const auto &n : force_nodes) {
                        sum += mesh->positions.row(n).y();
                      }
                      sum /= force_nodes.size();

                      // Skip negative displacements.
                      if (sum <= 0) {
                        continue;
                      }
#pragma omp critical
...
auto datasets::Deformation::DeformationBinarySearchReturnType::Ok() const
    -> bool {
  return utilities::numbers::IsApprox(displacement, target, epsilon);
}
auto datasets::Deformation::DeformationBinarySearchReturnType::TooLarge() const
    -> bool {
  return displacement > target;
}
auto datasets::Deformation::DeformationBinarySearchReturnType::TooSmall() const
    -> bool {
  return displacement < target;
}
