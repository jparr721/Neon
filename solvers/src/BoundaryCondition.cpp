...
void solvers::boundary_conditions::LoadBoundaryConditions(
    const Vector3r &force, const std::shared_ptr<meshing::Mesh> &mesh,
  ...
  for (const auto &index : force_nodes) {
    boundary_conditions.emplace_back(BoundaryCondition{index, force});
  }

  for (const auto &index : active_nodes) {
    boundary_conditions.emplace_back(
        BoundaryCondition{index, Vector3r::Zero()});
  }
}
