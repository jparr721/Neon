...
solvers::fem::LinearElastic::LinearElastic(
    ...
    : boundary_conditions(std::move(boundary_conditions)),
      mesh_(std::move(mesh)), type(type) {
  material_coefficients_ = materials::OrthotropicMaterial(
...
void solvers::fem::LinearElastic::SolveStatic() {
  ...
  if (lu_solver.info() != Eigen::Success) {
    NEON_LOG_ERROR("Failed to factorize using an LU factorization, falling "
                   "back to sparse QR (this could take a ",
                   "lot longer)");
    Eigen::SparseQR<SparseMatrixXr, Eigen::COLAMDOrdering<int>> qr_solver;
    qr_solver.compute(K_e);
    NEON_ASSERT_ERROR(qr_solver.info() == Eigen::Success,
                      "QR Solver failed! Check your stiffness matrix!");
...
auto solvers::fem::LinearElastic::AssembleElementStiffness() -> void {
  for (int row = 0; row < mesh_->tetrahedra.rows(); ++row) {
    ...
    // Prepare to compute the nodal stresses by transforming via the shape
    // functions and then computing the stress.
    const MatrixXr B = AssembleShapeFunctionMatrix(shape_one, shape_two,
                                                   shape_three, shape_four);
    const Real V = ComputeTetrahedralElementVolume(shape_one, shape_two,
                                                   shape_three, shape_four);
...
auto solvers::fem::LinearElastic::AssemblePlaneStresses(const MatrixXr &sigmas)
    -> MatrixXr {
...
auto solvers::fem::LinearElastic::ComputeShapeFunctionFromPoints(
    const Vector6r &points) -> Real {
...
auto solvers::fem::LinearElastic::AssembleShapeFunctionMatrix(
    const Vector3r &shape_one, const Vector3r &shape_two,
  ...
  const Real beta_1 =
      -1 * ComputeShapeFunctionFromPoints(
               (Vector6r() << y2, z2, y3, z3, y4, z4).finished());
  const Real beta_2 = ComputeShapeFunctionFromPoints(
      (Vector6r() << y1, z1, y3, z3, y4, z4).finished());
  const Real beta_3 =
      -1 * ComputeShapeFunctionFromPoints(
               (Vector6r() << y1, z1, y2, z2, y4, z4).finished());
  const Real beta_4 = ComputeShapeFunctionFromPoints(
      (Vector6r() << y1, z1, y2, z2, y3, z3).finished());

  const Real gamma_1 = ComputeShapeFunctionFromPoints(
      (Vector6r() << x2, z2, x3, z3, x4, z4).finished());
  const Real gamma_2 =
      -1 * ComputeShapeFunctionFromPoints(
               (Vector6r() << x1, z1, x3, z3, x4, z4).finished());
  const Real gamma_3 = ComputeShapeFunctionFromPoints(
      (Vector6r() << x1, z1, x2, z2, x4, z4).finished());
  const Real gamma_4 =
      -1 * ComputeShapeFunctionFromPoints(
               (Vector6r() << x1, z1, x2, z2, x3, z3).finished());

  const Real delta_1 =
      -1 * ComputeShapeFunctionFromPoints(
               (Vector6r() << x2, y2, x3, y3, x4, y4).finished());
  const Real delta_2 = ComputeShapeFunctionFromPoints(
      (Vector6r() << x1, y1, x3, y3, x4, y4).finished());
  const Real delta_3 =
      -1 * ComputeShapeFunctionFromPoints(
               (Vector6r() << x1, y1, x2, y2, x4, y4).finished());
  const Real delta_4 = ComputeShapeFunctionFromPoints(
      (Vector6r() << x1, y1, x2, y2, x3, y3).finished());
...
auto solvers::fem::LinearElastic::ComputeTetrahedralElementVolume(
    const Vector3r &shape_one, const Vector3r &shape_two,
