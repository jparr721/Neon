...
Homogenization::Homogenization(Tensor3r implicit_surface,
...
Homogenization::Homogenization(Tensor3r implicit_surface,
  ...
  lambda_ =
      Tensor3r(material_one_lambda.Instance() + material_two_lambda.Instance());
...
auto Homogenization::ComputeMaterialCoefficients() -> void {
  ...
  coefficients_.E_x = std::pow(S(0, 0), -1);
  coefficients_.E_y = std::pow(S(1, 1), -1);
  coefficients_.E_z = std::pow(S(2, 2), -1);

  coefficients_.G_yz = std::pow(S(3, 3), -1);
  coefficients_.G_zx = std::pow(S(4, 4), -1);
  coefficients_.G_xy = std::pow(S(5, 5), -1);
  ...
  coefficients_.coefficients << coefficients_.E_x, coefficients_.E_y,
      coefficients_.E_z, coefficients_.G_yz, coefficients_.G_zx,
      coefficients_.G_xy, coefficients_.v_yx, coefficients_.v_zx,
      coefficients_.v_zy, coefficients_.v_xy, coefficients_.v_xz,
      coefficients_.v_yz;
}
    ...
    for (int ii = 0; ii < xx.rows(); ++ii) {
      for (int jj = 0; jj < yy.rows(); ++jj) {
        for (int kk = 0; kk < zz.rows(); ++kk) {
          ...
          B << B_e.At(0), B_e.At(1), B_e.At(2), B_e.At(3), B_e.At(4), B_e.At(5),
              B_e.At(6), B_e.At(7);
...
auto Homogenization::ComputeElementDegreesOfFreedom(unsigned int n_elements)
    -> MatrixXi {
  NEON_ASSERT_ERROR(voxel_.Dimensions().size() == 3,
  ...
  NEON_ASSERT_ERROR(_nn.size() == number_of_nodes,
                    "Node numbers improperly formatted!", number_of_nodes);
...
auto Homogenization::ComputeUniqueNodes(unsigned int n_elements) -> Tensor3i {
  ...
  const unsigned int n_el_y = voxel_.Dimension(1);
  const unsigned int n_el_z = voxel_.Dimension(2);
  NEON_ASSERT_ERROR(n_el_x > 0 && n_el_y > 0 && n_el_z > 0,
                    "Voxel dims are zero!");
...
auto Homogenization::ComputeDisplacement(
    unsigned int n_degrees_of_freedom, const SparseMatrixXr &stiffness,
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      Real contribution_sum;
      {
        Tensor3r lambda_contribution =
            Tensor3r(lambda_.Instance() *
                     Tensor3r::Expand(sum_L, lambda_.Dimensions()).Instance());
        Tensor3r mu_contribution =
            Tensor3r(mu_.Instance() *
                     Tensor3r::Expand(sum_M, mu_.Dimensions()).Instance());

        contribution_sum = Tensor3r(lambda_contribution.Instance() +
...
void Homogenization::ComputeConstitutiveProduct(const MatrixXr &layeri,
                                                const MatrixXr &layerj,
                                                const MatrixXr &index,
