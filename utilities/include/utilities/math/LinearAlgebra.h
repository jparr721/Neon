...
template <typename T> using SparseVectorX = Eigen::SparseVector<T>;
...
inline auto ToTriplets(const VectorX<int> &i, const VectorX<int> &j,
  ...
  NEON_ASSERT_ERROR(Shape(i) == Shape(j) && Shape(i) == Shape(data),
                    "Shapes must match");
...
template <typename Derived>
inline auto Slice(const Eigen::DenseBase<Derived> &in, const int start,
                  const int end, Eigen::PlainObjectBase<Derived> &out) -> void {
  ...
  for (int i = start, out_idx = 0; i <= end; ++i, ++out_idx) {
    out(out_idx) = in(i);
  }
}

template <typename DerivedIn, typename DerivedOut, typename DerivedIndices>
inline auto Slice(const Eigen::DenseBase<DerivedIn> &in,
                  const Eigen::DenseBase<DerivedIndices> &rows,
                  const Eigen::DenseBase<DerivedIndices> &cols,
                  Eigen::PlainObjectBase<DerivedOut> &out) -> void {
  NEON_ASSERT_ERROR(rows.minCoeff() >= 0, "ROW INDEX IS LESS THAN 0");
  NEON_ASSERT_ERROR(rows.maxCoeff() <= in.rows(),
                    "ROW INDEX IS BIGGER THAN MAX SIZE OF INPUT MATRIX");
  NEON_ASSERT_ERROR(cols.minCoeff() >= 0, "COLUMN INDEX IS LESS THAN 0");
  NEON_ASSERT_ERROR(cols.maxCoeff() <= in.cols(),
...
template <typename T> auto Sort(VectorX<T> &out) -> void {
...
template <typename T> auto Dedupe(VectorX<T> &out) -> void {
  Sort(out);
  auto v = EigenVectorToSTLVector(out);
...
void NNZ(const Eigen::PlainObjectBase<DerivedIn> &in, VectorXi &I,
         unsigned int &count, bool rowwise) {
...
template <typename T>
void ComputeTriangleAngles(const Vector3<T> &a, const Vector3<T> &b,
                           const Vector3<T> &c, Real &A_hat, Real &B_hat,
                           Real &C_hat) {
  ...
  A_hat = std::acos((b_dist * b_dist + c_dist * c_dist - a_dist * a_dist) /
                    (2 * (b_dist * c_dist)));
  B_hat = std::acos((a_dist * a_dist + c_dist * c_dist - b_dist * b_dist) /
                    (2 * (a_dist * c_dist)));
  C_hat = std::acos((b_dist * b_dist + a_dist * a_dist - c_dist * c_dist) /
                    (2 * (b_dist * a_dist)));
}
...
template <typename Derived>
auto Contains(const Eigen::MatrixBase<Derived> &in,
              const typename Derived::Scalar value) -> bool {
...
inline auto IsApprox(const Real lhs, const Real rhs, const Real epsilon)
    -> bool {
  return std::abs(lhs - rhs) < epsilon;
}

inline auto Degrees(const Real degree) -> Real {
  return degree * (180.0 / kPi);
}

inline auto Radians(const Real radian) -> Real {
  return radian * (kPi / 180.0);
}

template <typename T>
inline void FastDiagonalInverse(const SparseMatrixX<T> &in,
                                SparseMatrixX<T> &out) {
  using triplet = Eigen::Triplet<T>;

  std::vector<triplet> entries;
  out.resize(in.rows(), in.cols());
  // If it's identity with some arbitrary coefficient, the inverse is just the
  // reciprocal of the first nonzero value
